      ...
      <main className="p-6 -mt-10 max-w-2xl mx-auto space-y-6">
        <div className="bg-white p-8 rounded-[40px] shadow-xl space-y-6">
          <div>
            ...
            <select className="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-3xl text-lg appearance-none">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 gap-3">
              {['Consulta Médica', 'Vacinação', 'Retirada de Exames'].map((item) => (
                <button key={item} className="p-5 border-2 border-slate-100 rounded-3xl text-left hover:border-blue-500 hover:bg-blue-50 transition-all font-medium text-slate-700">
                  {item}
                </button>
              ))}
            </div>
          ...
          <button className="w-full bg-emerald-500 text-white py-6 rounded-3xl font-black text-2xl shadow-xl hover:bg-emerald-600 transition-all">
            GERAR SENHA DIGITAL
          </button>
        </div>
      </main>
